...
from FAKEBOB_enhance_2 import FakeBob_enhance_2
...
def main(spk_id_list, architecture, task, threshold, attack_type, adver_thresh,
         ...
         momentum, plateau_length, plateau_drop, per_thresh, n_jobs, debug):
    ...
    fake_bob = FakeBob_enhance_2(task, attack_type, model, adver_thresh=adver_thresh, epsilon=epsilon, max_iter=max_iter,
                         max_lr=max_lr, min_lr=min_lr, samples_per_draw=samples_per_draw, sigma=sigma, momentum=momentum, 
                         plateau_length=plateau_length, plateau_drop=plateau_drop, per_thresh=per_thresh)
...
if __name__ == "__main__":
    ...
    # Add new threshold
    parser.add_argument("--per_thresh", "-k", type=float)
